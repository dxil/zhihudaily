...
<template>
  ...
  <div class="detail-main-box">
    <div v-if="image" class="detail-img-box mt" :style="{backgroundImage: 'url(' + replace(image) + ')'}">
    ...
    <!--推荐者部分-->
    <div v-link="{path: '/recommender' }" v-if="recommenders.length > 0" class="recommenders-box">
      <p>推荐者</p>
    ...
    route: {
      activate (transition) {
        ...
        if (transition.from.path === '/') {
          // 对window 中的sessionStorage 设置key value值,comments评论路由中会用到
