<template>
  <div>
    ...
    <ul class="long-comments">
      ...
      <div class="no-comments" v-if="extra.long_comments === 0">
...
<script>
  ...
  export default{
    ...
    route: {
      activate (transition) {
        var _this = this
        _this.detailId = window.sessionStorage.detailId   // 之前把文章id存入了缓存,现在取出来
    ...
    methods: {
      ...
      getShortComments () {
        ...
        ajax({
          ...
          callback: function (res) {
            ...
            // 这个id用来获取当前页面最后一条短评，但翻阅到底部后，继续刷新 获取接下来的短评
      ...
      getShortCommentsNext () {
        ...
        _this.loading = true
        ajax({
          url: 'http://news-at.zhihu.com/api/4/story/' + _this.detailId + '/short-comments/before/' + _this.id,
          method: 'GET',
          callback: function (res) {
