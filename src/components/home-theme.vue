...
<script>
  ...
  export default{
    ...
    route: {
      data (transition) {
        var _this = this
        // 判断从哪里来，如果来自详情页就不重新请求，并返回上次列表位置
    ...
    methods: {
      getTheme () {
        ...
        ajax({
          ...
          url: 'http://news-at.zhihu.com/api/4/theme/' + _this.$route.params.id,
          method: 'GET',
      ...
      getThemeBefore () {
        ...
        ajax({
          ...
          url: 'http://news.at.zhihu.com/api/4/theme/' + _this.$route.params.id + '/before/' + _this.id,
          method: 'GET',
          callback: function (res) {
